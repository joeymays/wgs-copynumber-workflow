# Xin Zhao's modifications to CopywriteR plotCNA function for Copy Number Calling
...
plotCNA <- function(destination.folder, smoothed = TRUE, sample.plot, y.min,
  ...
  save(segment.CNA.object, file = file.path(destination.folder,
                                            "segment.Rdata"))
  for (i in 1:nrow(segment.CNA.object$output)){
  if(segment.CNA.object$output$seg.mean[i] >2){
    segment.CNA.object$output$seg.mean[i] <- 1.5
  }else if(segment.CNA.object$output$seg.mean[i] < (-2)){
    segment.CNA.object$output$seg.mean[i] <- (-1.5)
  }else if(segment.CNA.object$output$seg.mean[i] > (-0.3) & segment.CNA.object$output$seg.mean[i] < (0.3)){
    segment.CNA.object$output$seg.mean[i] <- 0
  }
  }
                     ...
                     function(x) {
                       ...
                       plot(current.sample$data[, "maploc"],
                            ...
                            pch = ".", cex = 1, xlim = c(genome.position.min, 
                                                         genome.position.max),
                            xaxs = "i", xlab = "", ylab = "Log2 Copy Number Change", xaxt = "n",
                            main = select.sample,col="grey",cex.lab=1.3)
                       ...
                       invisible(apply(current.sample$output, 1, function(x) {
                         segments(x0 = as.numeric(x["loc.start"]),
                                  ...
                                  col = ifelse(as.numeric(x["seg.mean"])>=0.3,"red",ifelse(-0.3>=as.numeric(x["seg.mean"]),"blue","black")), pch = ".", cex = 6,lwd = 6)
                       }))
                       par(xpd = TRUE)
