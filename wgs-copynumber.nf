...
process COPYWRITER200 {
        ...
        publishDir "copywriter-output/${sample_id}/copynumber-200kb", mode: 'copy', pattern: '*/results-200kb/**/*.pdf', saveAs: { "${sample_id}_${file(it).name}" }
        ...
        singularity exec --home $PWD:/mnt/ --bind !{params.scriptsPath}:/mnt/scripts/ !{params.singularityPath} Rscript scripts/copywriter-parallel-nf.R --bam.file=!{bam} --species=!{params.species} --res="200kb"
        '''
...
process COPYWRITER1000 {
        ...
        publishDir "copywriter-output/${sample_id}/copynumber-1000kb", mode: 'copy', pattern: '*/results-1000kb/**/*.pdf', saveAs: { "${sample_id}_${file(it).name}" }
