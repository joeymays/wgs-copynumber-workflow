...
process COPYWRITER1000 {
	...
	publishDir "copywriter-output/${sample_id}/copynumber-1000kb", mode: 'copy', pattern: '*/results-1000kb/**/*.pdf', saveAs: { "${file(it).name}" }
        ...
        singularity exec --home $PWD:/mnt/ --bind !{params.scriptsPath}:/mnt/scripts/ !{params.singularityPath} Rscript scripts/copywriter-nf.R --bam.file=!{bam} --species=!{params.species} --res="1000kb"
        '''
